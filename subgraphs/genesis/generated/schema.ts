...
export class SimplePostSwapPool extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save SimplePostSwapPool entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type SimplePostSwapPool must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("SimplePostSwapPool", id.toString(), this);
    }
  }

  static loadInBlock(id: string): SimplePostSwapPool | null {
    return changetype<SimplePostSwapPool | null>(
      store.get_in_block("SimplePostSwapPool", id)
    );
  }

  static load(id: string): SimplePostSwapPool | null {
    return changetype<SimplePostSwapPool | null>(
      store.get("SimplePostSwapPool", id)
    );
  }
  ...
  set timestampUnix(value: i32) {
    this.set("timestampUnix", Value.fromI32(value));
  }
}
